TicTacToe-MinMax

A simple, web-based Tic-Tac-Toe game powered by the **Min-Max algorithm**, originally created as a university class project. 

## Modified Rules
Please note that this game uses a reversed rule set. Unlike standard Tic-Tac-Toe, the player who places three of their marks in a row (horizontally, vertically, or diagonally) loses the game.
The game randomly decides whether the player or the AI goes first.
## Technologies Used
* HTML5
* CSS3
* Vanilla JavaScript

## How to Play
1. Clone this repository or download the files.
2. Open `index.html` in your favorite web browser.
3. Click **"Rozpocznij Grę!"** to begin.
4. Try to force the AI to make a line of three `X`s or `O`s before you do!

## Acknowledgments
* The Minimax algorithm implementation was inspired by [The Coding Train's Tic Tac Toe Minimax Challenge (CC 154)](https://github.com/CodingTrain/website/tree/main/CodingChallenges/CC_154_Tic_Tac_Toe_Minimax/P5).
